...
export function ChatWindow({ chatId, currentUser, otherUser, isGroup = false, groupData, onBack }: ChatWindowProps) {
    ...
    const startRecording = async () => {
        ...
        try {
            ...
            if (!getUserMedia) {
                throw new Error('Voice recording is not supported in this browser or connection context.\n\n⚠️ IMPORTANT FOR MOBILE TESTING:\nModern mobile browsers completely block microphone/camera access on unsecure HTTP connections.\n\nTo test this on your mobile device, please access the site using a secure HTTPS connection (e.g. by using a tunnel like ngrok or localtunnel).');
            }

            if (typeof MediaRecorder === 'undefined') {
                throw new Error('MediaRecorder API is not supported in this browser. Please use a modern browser.');
            }
            ...
            if (err.message && (err.message.includes('MOBILE TESTING') || err.message.includes('MediaRecorder API'))) {
                alert(err.message);
            } else {
    ...
    const handleUnsendMessage = async (message: Message, mode: 'me' | 'everyone') => {
        ...
        try {
            ...
            // Perform the message update/deletion
            if (mode === 'everyone') {
            ...
            // Check if this was the last message to update the parent chat/group preview
            const isLastMessage = messages.length > 0 && messages[messages.length - 1].id === message.id;
            if (isLastMessage) {
                const parentRef = doc(db, isGroup ? 'groups' : 'chats', chatId);
                if (mode === 'everyone') {
                    await updateDoc(parentRef, {
                        lastMessage: '🚫 Message unsent'
                    });
                } else {
                    // Deleted for me: find previous message in the list
                    if (messages.length > 1) {
                        ...
                        if (prevMsg.isDeleted) {
                            ...
                            const decryptedText = decryptMessage(prevMsg.text || '', encryptionSecret);
                            if (decryptedText) {
                                previewText = decryptedText;
                            } else if (prevMsg.audioUrl) {
                                previewText = '🎙️ Voice message';
                            } else if (prevMsg.fileUrl) {
                                previewText = '📄 File';
                            } else if (prevMsg.imageUrl) {
                                previewText = '📷 Photo';
                            } else if (prevMsg.videoUrl) {
                                previewText = '🎥 Video';
                            } else if (prevMsg.poll) {
                                previewText = '📊 Poll';
                            } else {
                                previewText = 'Sent a secure message';
                            }
                        }
                        await updateDoc(parentRef, {
                            lastMessage: previewText,
                            lastMessageAt: prevMsg.createdAt || serverTimestamp()
                        });
                    } else {
        ...
        return (
            <div className="flex-1 flex flex-col items-center justify-center p-8 md:p-12 text-center bg-white/40 dark:bg-black/5 animate-fade-in gap-6">
                ...
                <div className="flex items-center gap-3 px-6 py-3 bg-brand-burgundy/5 rounded-2xl border border-brand-burgundy/10 shadow-inner group transition-all hover:bg-brand-burgundy/10">
                    ...
                    <span className="text-[10px] md:text-[11px] font-bold uppercase tracking-[0.15em] text-brand-burgundy/60">
        ...
        <div className="flex-1 flex flex-col h-full bg-white/20 dark:bg-brand-parchment/5 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between px-8 py-3 bg-white dark:bg-brand-cream border-b border-brand-ebony/10 dark:border-white/10 z-30 sticky top-0 shadow-sm">
                ...
                <div className="flex items-center gap-2 shrink-0">
                    <div className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 bg-brand-ebony/[0.03] rounded-full mr-2 ring-1 ring-brand-ebony/5">
                        <Lock className="w-3.5 h-3.5 text-brand-ebony/40" />
                        <span className="text-[9px] font-bold text-brand-ebony/40 uppercase tracking-widest whitespace-nowrap">Encrypted</span>
                    </div>

                    <button 
                        onClick={() => alert('Voice calling coming soon!')}
                        className={`p-3 rounded-xl transition-all shadow-sm border bg-white dark:bg-white/5 text-brand-burgundy border-white hover:bg-brand-burgundy/5 hover:border-brand-burgundy/20`}
                    >
            ...
            {/* Messages Area */}
            <div
                className="flex-1 overflow-y-auto px-4 md:px-12 pt-6 pb-[160px] md:pb-36 space-y-5 scrollbar-hide bg-[#f2f4f7] dark:bg-[#0a0a0c]"
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full space-y-8 animate-fade-in max-w-sm mx-auto">
                         {isGroup ? (
                            ...
                            <img src={profilePic || ''} alt={title || 'Profile'} className="w-24 h-24 rounded-[2rem] object-cover shadow-2xl border-4 border-white dark:border-brand-parchment ring-1 ring-brand-ebony/5" />
                        )}
                        <div className="text-center">
                            <p className="text-xl font-serif font-extrabold text-brand-ebony mb-3">Say hello to {title}</p>
                            <div className="flex items-center justify-center gap-2 text-emerald-500/60 font-bold uppercase tracking-widest text-[10px]">
                        ...
                        {showAttachmentMenu && (
                            <div className="absolute left-0 bottom-full mb-6 w-72 card-premium shadow-2xl z-[110] overflow-hidden animate-in fade-in slide-in-from-bottom-6">
                                <div className="p-3 grid grid-cols-2 gap-2">
                                    {[
                                        { icon: ImageIcon, label: 'Photo', sub: 'Images', type: 'image', ref: fileInputRef, color: 'text-blue-500', bg: 'bg-blue-50' },
                                        { icon: VideoIcon, label: 'Video', sub: 'Motion', type: 'video', ref: videoInputRef, color: 'text-violet-500', bg: 'bg-violet-50' },
                                        { icon: FileText, label: 'Document', sub: 'Files', type: 'file', ref: docInputRef, color: 'text-amber-500', bg: 'bg-amber-50' },
                                        { icon: BarChart2, label: 'Poll', sub: 'Voting', action: () => { setShowAttachmentMenu(false); setShowPollModal(true); }, color: 'text-emerald-500', bg: 'bg-emerald-50' }
                                    ].map((item, idx) => (
                                        <button 
                                            key={idx}
                    ...
                    <form onSubmit={handleSendMessage} className="flex-1 flex items-center gap-4">
                        <div className="flex-1 relative group">
                            ...
                            >
                                ...
                                >
                                    {/(\u00a9|\u00ae|[\u2000-\u3300]|\ud83c[\ud000-\udfff]|\ud83d[\ud000-\udfff]|\ud83e[\ud000-\udfff])/.test(newMessage) 
                                        ? <EmojiRenderer text={newMessage || ' '} /> 
                                        : (newMessage || ' ')}
                                    {/* Placeholder if empty */}
                                    {!newMessage && <span className="text-brand-ebony/30 absolute left-5">{editingMessage ? 'Revising message...' : (replyingToMessage ? 'Drafting response...' : 'Message...')}</span>}
                                </div>
                            ...
                            {isRecording ? (
                                ...
                                <div className="flex items-center gap-2 animate-in fade-in slide-in-from-right-2 duration-300">
                                    ...
                                    <button
                                        ...
                                        className="p-3 rounded-full bg-white dark:bg-white/5 text-red-500 border border-red-200 hover:bg-red-50 hover:border-red-300 transition-all shadow-sm flex items-center justify-center shrink-0 active:scale-90"
                                    ...
                                    <div className="flex items-center gap-2 px-3 py-2 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-full">
                                        <div className="relative flex h-2.5 w-2.5">
                                            <div className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></div>
                                            <div className="relative inline-flex rounded-full h-2.5 w-2.5 bg-red-500"></div>
                                        </div>
                                        <span className="text-xs font-bold text-red-600 dark:text-red-400 tabular-nums min-w-[36px]">
                                    ...
                                    <button
                                        ...
                                        className="p-3 bg-gradient-to-br from-brand-burgundy to-[#4a1c1f] text-white rounded-full shadow-premium hover:brightness-110 active:scale-95 transition-all flex items-center justify-center shrink-0"
                                    >
                                ...
                                <>
                                    <button
                                        ...
                                        className="p-3 rounded-full transition-all flex items-center justify-center flex-shrink-0 shadow-lg bg-white dark:bg-white/5 text-emerald-500 border border-white hover:bg-emerald-50 hover:border-emerald-100"
                                    >
